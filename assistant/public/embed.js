(() => {
  ...
  function renderSuggestion(data) {
    suggestion = data;
    const sources = Array.isArray(data.sources) && data.sources.length ? `<div class="sources"><b>Base consultada:</b> ${data.sources.map(item => `${escapeHtml(item.title)} (v${escapeHtml(item.version)})`).join(' · ')}</div>` : '';
    suggestionsView.innerHTML = `<div class="card ai-row"><div class="spark">✦</div><div class="ai-copy">Analisei a conversa e consultei a base comercial aprovada. Preparei uma resposta para sua revisão.</div></div><div class="card"><div class="section-title">Resposta sugerida</div><div class="reply">${escapeHtml(data.suggestedReply)}</div>${sources}<div class="actions"><button class="btn primary insert" type="button">Inserir no campo</button><button class="btn copy" type="button">Copiar</button><button class="btn outline regenerate" type="button">Gerar outra</button></div></div><div class="card summary"><div class="section-title">Resumo da conversa</div><div>${escapeHtml(data.summary)}</div><div>• <b>Interesse:</b> ${escapeHtml(data.interest)}</div><div>• <b>Empresa:</b> ${escapeHtml(data.company)}</div><div>• <b>Próxima ação:</b> ${escapeHtml(data.nextAction)}</div></div>`;
